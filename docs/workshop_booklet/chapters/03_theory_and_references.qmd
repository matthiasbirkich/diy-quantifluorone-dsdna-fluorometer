...
**PCR target:** For the downstream PCR workflow, approximately 100 fmol DNA template is targeted, with 80 fmol used as the minimum acceptable input. Because molar amount depends on DNA fragment length, conversion from the fluorometrically measured mass concentration requires an appropriate representative fragment length:

$$

n_{\mathrm{DNA}}\;(\mathrm{fmol}) \approx

\frac{m\;(\mathrm{ng})\times10^6}

{650\times L\;(\mathrm{bp})}

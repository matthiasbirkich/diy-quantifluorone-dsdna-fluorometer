...
Open Colorimeter Plus project [@iorodeo_ocp_firmware].

The installed LED driver is the ioRodeo fixed-current radial 16 mA LED board,
revision `ver_0p1_rev_3` [@iorodeo_led_board]. For integration into the optical
module, the through-hole LED is mounted on the rear side of the board. The
board design files, licence and source record are retained in the repository
for traceability.
...
Further examples of open photometric and fluorometric DNA quantification
using ioRodeo instrumentation are documented by ioRodeo
[@iorodeo_uv_dna_quantification; @iorodeo_ocp_dna_quantitation;
@iorodeo_ocp_updates_2025].

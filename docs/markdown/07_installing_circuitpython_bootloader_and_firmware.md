...
The actual reset switch is mounted on the underside of the PyBadge. For every reset action, turn the instrument over and press the button from below through the dedicated underside access opening; it is not operated from the front-panel side.
...
![Underside access to the PyBadge reset button. Use a short non-conductive tool and press the switch from below through the dedicated opening.](../figures/photos/ch07/reset_button_access.png)

## 3. Bootloader mode

Connect USB, switch the device on, turn the instrument over, and fast-double-click the underside Reset button. The display changes to the PyBadge UF2 bootloader screen and the computer mounts `PYBADGEBOOT` or `BADGEBOOT`.
...
Safe mode prevents the QuantiFluorONE application from running while the computer modifies the filesystem. After safely ejecting the drive, turn the instrument over and press the underside Reset button once to return to normal operation.

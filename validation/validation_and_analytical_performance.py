#!/usr/bin/env python3
"""Reproduce the Quantus–DIY comparison statistics and core validation figures.

The script reads the paired Quantus and DIY-QuantiFluorONE validation
dataset, calculates regression and agreement statistics, exports the
numerical results, and regenerates the corresponding comparison figures.

Blank-based LOD and LOQ values used in the summary figure are currently
precomputed validation results and are not recalculated by this script.
...
DATA_FILE = VALIDATION_DIR / "quantus_diy_two_point_comparison.csv"
RESULTS_FILE = VALIDATION_DIR / "validation_statistics.csv"
...
mard = float(
    np.mean(
        np.abs(
            (d_ind[quantitative] - q_ind[quantitative])
            / q_ind[quantitative]
        )
    )
    * 100
...
ax.scatter(
    q_ind,
    d_ind,
    label="Independent validation results",
)
...
ax.scatter(
    ...
    marker="s",
    s=80,
    label="Two-point calibration anchor",
)
...
ax.plot(
    xline,
    slope_ind * xline + intercept_ind,
    label="Regression, calibration anchor excluded",
)

ax.set_xlabel("Quantus comparison concentration (ng/µL)")
ax.set_ylabel("Two-point-calibrated DIY fluorometer result (ng/µL)")
ax.set_title(
    "Linearity of the two-point-calibrated DIY fluorometer\n"
    "0–400 ng/µL dsDNA"
)
...
fig.savefig(
    FIG_DIR / "two_point_calibrated_diy_linearity_0_400.png",
    dpi=300,
)
...
ax.axhline(
    mean_bias,
    label=f"Mean bias = {mean_bias:.2f} ng/µL",
)

ax.axhline(
    upper_limit,
    linestyle="--",
    label=f"Upper 95% limit = {upper_limit:.2f}",
)

ax.axhline(
    lower_limit,
    linestyle="--",
    label=f"Lower 95% limit = {lower_limit:.2f}",
)
...
fig.savefig(
    FIG_DIR / "quantus_diy_bland_altman.png",
    dpi=300,
)
...
fig.savefig(
    FIG_DIR / "blank_based_lod_loq_summary.png",
    dpi=300,
)
...
with RESULTS_FILE.open(
    ...
    writer.writerows(
        [
            [
                "independent_linearity",
                "n",
                len(q_ind),
                "",
                "Calibration anchor excluded",
            ],
            [
                "independent_linearity",
                "slope",
...
# validation/diy_triplicate_concentration_summary.csv,
# which expresses the three technical TSL2591 readings of each cycle
...
# The updated Chapter 10 figures use smaller markers so that error bars
# remain visible, and they add the 95% confidence band of the independent
# linear regression. The Bland–Altman plot is included as an agreement
# assessment complementary to, not replacing, the linearity plot.